...
def get_event_details(
    ...
    *,
    ...
    """
    try:
        if known_titles and event_url.strip().lower() in known_titles:
            info(logger, "bol.info.ignorar_conhecido", url=event_url)
            return None

        session.headers.update(get_random_headers())
        resp = _http_get(event_url, timeout=REQUESTS_TIMEOUT)
        ...
        if credits_tag:
            ...
            credits = p.get_text(strip=True) if p else "N/A"
...
def scrape_theatre_info(known_titles: Optional[set[str]] = None) -> pd.DataFrame:
    ...
    with requests.Session() as session:
        ...
        for url in urls:
            ...
            familia_whitelist = {"Teatro", "Musical"} if is_familia else None
